...
database
#counter for the images shown
# main() function of the imageRetrieve class retrieves all the images from the
...
self.close()
self.recover = Ui_Recovery(self.uname,self.code)
#recovery code and username are passed as
arguments
self.recover.show()
except Error as e:
errorstr = str(e)
...
self.cursor.close()
self.db.close()
#send_mail function sends the recovery text from iamnotsparsha@gmail.com to the user Email def send_mail(self,userEmail):
#gmail connection
try:
...
#******************first the username is fetched from the Database************************* self.uname=str(self.lineEdit.text())
if not self.uname:
...
'''*************if the username is matched, user details like number of images(inum), tolerance value(tvalue),list of images(imageList) and clickpoints(cpx,cpy) are fetched from the database************************'''
self.inum=rows[5]
self.tvalue=rows[6]
self.imageList=[]
self.cpx=[]
self.cpy=[]
#list of user images
#list of x-coordinates for clickpoints
#list of Y-coordinates for clickpoints
...
correct image is shown and remaining entities are closed***********''' self.gencorrectImage()
self.lineEdit.close()
self.okay.close()
...
self.cursor.close()
self.db.close()
#if the user clicks on correct series of clickpoints, correct series of images are generated def gencorrectImage(self):
...
#don't remember what purpose this line of statement served
#f the user clicks on incorrect clickpoints, random series of images are displayed def genrandomImage(self):
self.clickpoint.close()
...
#if the user gets logged in then a new window, 'home' is shown def openHome(self,cpx,cpy):
